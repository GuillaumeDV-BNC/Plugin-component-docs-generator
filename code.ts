type Params = { status: string; dsLink: string; markdownDocs: string; generatedOverview: string; variantCols: number; githubUrl?: string; githubSha?: string }
type Attachment = { version: 1; params: Params; state: unknown | null }
type RunMsg =
  | { type: 'action'; id: string; params: Partial<Params> }
  | { type: 'resize'; height: number }
  | { type: 'get-github-info' }
  | { type: 'save-gh-token'; token: string }
  | { type: 'get-gh-token' }
// layoutSizingHorizontal / layoutSizingVertical are not in the public type defs
// but exist at runtime. Must be set AFTER appendChild — Figma enforces this.
type WithLayoutSizing = {
...
const DEFAULTS: Params = { status: 'Ready', dsLink: '', markdownDocs: '', generatedOverview: '', variantCols: 1 }
...
function normalizeParams(input: Partial<Params> | null | undefined): Params {
  ...
  return {
    ...
    markdownDocs:      typeof v.markdownDocs      === 'string' ? v.markdownDocs      : DEFAULTS.markdownDocs,
    generatedOverview: typeof v.generatedOverview === 'string' ? v.generatedOverview : DEFAULTS.generatedOverview,
    variantCols:       typeof v.variantCols       === 'number' ? Math.max(1, Math.min(4, Math.round(v.variantCols))) : DEFAULTS.variantCols,
  }
}
...
function readAttachment(node: SceneNode): Attachment | null {
  try {
    const p = JSON.parse(node.getPluginData(ATTACH_KEY)) as Partial<Attachment>
    if (p?.version !== 1) return null
    return { version: 1, params: normalizeParams(p.params), state: (p.state ?? null) as unknown }
  } catch { return null }
}
...
// Markdown parser
// ─────────────────────────────────────────────────────────────────────────────
function mdSection(md: string, keys: string[]): string {
  if (!md) return ''
  const lines = md.split('\n')
  for (const key of keys) {
    const esc = key.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')
    let on = false; let parentLevel = 1; const out: string[] = []
    for (const line of lines) {
      const startMatch = line.match(new RegExp(`^(#+)\\s*${esc}\\s*$`, 'i'))
      if (startMatch) { on = true; parentLevel = startMatch[1].length; continue }
      if (on) {
...
function mdBullets(md: string, keys: string[]): string[] {
  const s = mdSection(md, keys)
  if (!s) return []
  return s.split('\n').map(l => l.replace(/^[-*+]\s*/,'').trim()).filter(l => l.length > 0)
}

function _mdDosDonts(md: string): { dos: string[]; donts: string[] } {
  const dos   = mdBullets(md, ['Do', 'Dos', "Do's", 'Should', 'Best practice'])
  const donts = mdBullets(md, ["Don't","Donts","Don'ts",'Avoid','Do not','Should not'])
  const combined = mdSection(md, ["Dos and Don'ts","Do's and Don'ts","Do / Don't","Do's & Don'ts"])
  if (combined) {
    const lines = combined.split('\n'); let inDo = false; let inDont = false
    for (const line of lines) {
      if (line.match(/^#+\s*(do|dos|do's|should)\b/i))          { inDo = true; inDont = false; continue }
      if (line.match(/^#+\s*(don'?t|avoid|do not|should not)/i)) { inDont = true; inDo = false; continue }
...
interface CProp { rawKey: string; type: string; values: string[]; defaultValue: string }

function classifyProp(name: string, values: string[]): 'appearance'|'size'|'variant' {
...
// Action
// ─────────────────────────────────────────────────────────────────────────────
function evaluateEnabled_generate(selection: readonly SceneNode[]): boolean {
...
async function action_generate(
  params: Params,
  target: SceneNode,
  _prev: unknown | null
): Promise<{ affectedNodes: SceneNode[]; state: unknown | null }> {
  const affectedNodes: SceneNode[] = [target]
  const selection = figma.currentPage.selection

    const sel = selection[0]
    if (!sel) { figma.notify('Select a component, component set, or instance first'); return { affectedNodes, state: null } }
    ...
    if (sel.type === 'INSTANCE') {
      mainComp = await (sel as InstanceNode).getMainComponentAsync()
      if (mainComp?.parent?.type === 'COMPONENT_SET') compSet = mainComp.parent as ComponentSetNode
    } else if (sel.type === 'COMPONENT') {
      mainComp = sel as ComponentNode
      if (mainComp.parent?.type === 'COMPONENT_SET') compSet = mainComp.parent as ComponentSetNode
    } else if (sel.type === 'COMPONENT_SET') {
      compSet  = sel as ComponentSetNode
      mainComp = (compSet.children[0] as ComponentNode) ?? null
    } else {
      figma.notify('Select a component, component set, or instance first'); return { affectedNodes, state: null }
    }
    ...
    if (compSet) {
      for (const child of compSet.children) {
        ...
        for (const [k,v] of Object.entries(parseVariantName(child.name))) {
          ...
          const a = propMap.get(k)!; if (!a.includes(v)) a.push(v)
        }
      }
    }
    ...
    function isBoolLike(values: string[]): boolean {
      return values.length === 2 && values.every(v => BOOL_VAL_RE.test(v))
    }
    // Canonical "true" value for a boolean-like variant
    function boolTrueVal(values: string[]): string {
      return values.find(v => /^(true|yes|on|visible|enabled|show)$/i.test(v)) ?? values[1] ?? 'true'
    }
    function boolFalseVal(values: string[]): string {
      return values.find(v => /^(false|no|off|hidden|disabled|hide)$/i.test(v)) ?? values[0] ?? 'false'
    }

    const componentProps = new Map<string,CProp>()
    const src = compSet ?? mainComp
    if (src && 'componentPropertyDefinitions' in src) {
      for (const [rawKey, def] of Object.entries(src.componentPropertyDefinitions)) {
        // Strip everything from '#' onwards — handles formats like "showTitle#1212:0"
        const k = rawKey.split('#')[0].trim()
        if (def.type === 'VARIANT') {
          const vals = (def as {type:'VARIANT';variantOptions?:string[];defaultValue:string}).variantOptions ?? propMap.get(k) ?? []
          // Treat VARIANT with only true/false-like values as BOOLEAN
          const effectiveType = isBoolLike(vals) ? 'BOOLEAN' : 'VARIANT'
          componentProps.set(k, { rawKey, type: effectiveType, values: vals, defaultValue: String(def.defaultValue) })
        } else if (def.type === 'BOOLEAN') {
          componentProps.set(k, { rawKey, type:'BOOLEAN', values:['false','true'], defaultValue: String(def.defaultValue) })
        } else if (def.type === 'TEXT') {
          componentProps.set(k, { rawKey, type:'TEXT', values:[], defaultValue: String(def.defaultValue) })
        }
      }
    }
    if (componentProps.size === 0) {
      for (const [k,vals] of propMap) {
        const effectiveType = isBoolLike(vals) ? 'BOOLEAN' : 'VARIANT'
        componentProps.set(k, { rawKey:k, type: effectiveType, values:vals, defaultValue:vals[0]??'' })
      }
    }

    const appearanceProps: [string,CProp][] = []
    const sizeProps:       [string,CProp][] = []
    const booleanProps:    [string,CProp][] = []
    const otherProps:      [string,CProp][] = []
    ...
    const themeProps: [string,CProp][] = []
    function isThemeProp(name: string, data: CProp): boolean {
      if (data.type !== 'VARIANT' || data.values.length < 2) return false
      if (THEME_PROP_RE.test(name)) return true
      return data.values.length >= 2 && data.values.every(v => THEME_VAL_RE.test(v))
    }
    function extractThemeFromBucket(bucket: [string,CProp][]): void {
      for (let i = bucket.length - 1; i >= 0; i--) {
        const [n, d] = bucket[i]
        if (isThemeProp(n, d)) { themeProps.push([n, d]); bucket.splice(i, 1) }
      }
    }
    ...
    function findBestComponent(propName: string, propValue: string): ComponentNode | null {
      if (!compSet) return mainComp
      const cs = (compSet.children as ComponentNode[]).filter(
        c => c.type === 'COMPONENT' && parseVariantName(c.name)[propName] === propValue
      )
      if (!cs.length) return null
      if (cs.length === 1) return cs[0]
      let best = cs[0]; let score = -1
      for (const c of cs) {
        const p = parseVariantName(c.name); let s = 0
    ...
    // ── Fonts ─────────────────────────────────────────────────────────────────
    type FontDef = { family: string; style: string }
    let fontFamily = 'Inter'
    for (const fam of ['Inter','Roboto','SF Pro Text','Helvetica Neue']) {
      try { await figma.loadFontAsync({ family: fam, style: 'Regular' }); fontFamily = fam; break } catch { /* try next */ }
    }
    async function lf(styles: string[]): Promise<FontDef> {
      for (const s of styles) {
        try { await figma.loadFontAsync({ family: fontFamily, style: s }); return { family: fontFamily, style: s } } catch { /* try next */ }
      }
    ...
    // ── Colors ────────────────────────────────────────────────────────────────
    const cBg:     RGB = { r:0.975, g:0.975, b:0.985 }
    const cSurf:   RGB = { r:1,     g:1,     b:1     }
    const cBorder: RGB = { r:0.87,  g:0.87,  b:0.91  }
    const cText:   RGB = { r:0.08,  g:0.08,  b:0.10  }
    const cMuted:  RGB = { r:0.42,  g:0.42,  b:0.48  }
    const cSecBg:  RGB = { r:0.94,  g:0.94,  b:0.96  }
    const cPrev:   RGB = { r:0.96,  g:0.96,  b:0.985 }
    const cGreen:  RGB = { r:0.07,  g:0.53,  b:0.31  }
    const cRed:    RGB = { r:0.85,  g:0.19,  b:0.19  }
    const cBlue:   RGB = { r:0.09,  g:0.44,  b:0.97  }
    const STATUS_COLORS: Record<string,{bg:RGB;text:RGB}> = {
      'Draft':       { bg:{r:0.88,g:0.88,b:0.92}, text:{r:0.35,g:0.35,b:0.42} },
      'In Progress': { bg:{r:1.00,g:0.94,b:0.80}, text:{r:0.62,g:0.43,b:0.05} },
      'Ready':       { bg:{r:0.85,g:0.96,b:0.88}, text:{r:0.08,g:0.50,b:0.28} },
      'Deprecated':  { bg:{r:1.00,g:0.90,b:0.82}, text:{r:0.68,g:0.30,b:0.07} },
      'Archived':    { bg:{r:0.88,g:0.88,b:0.92}, text:{r:0.35,g:0.35,b:0.42} },
    }

    function sp(c: RGB, a = 1): SolidPaint { return { type:'SOLID', color:c, opacity:a } }

    // ── Layout helpers ────────────────────────────────────────────────────────
    function addFill(parent: FrameNode, child: SceneNode | TextNode): void {
      parent.appendChild(child)
      ;(child as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'
    }

    function add(parent: FrameNode, child: SceneNode | TextNode): void {
      parent.appendChild(child)
    }

    function mkV(name: string, gap = 0): FrameNode {
      const f = figma.createFrame()
      f.name = name; f.fills = []; f.layoutMode = 'VERTICAL'
      f.primaryAxisSizingMode = 'AUTO'
      f.counterAxisSizingMode = 'AUTO'
      f.itemSpacing = gap; f.clipsContent = false
      return f
    }

    function mkH(name: string, gap = 0): FrameNode {
      const f = figma.createFrame()
      f.name = name; f.fills = []; f.layoutMode = 'HORIZONTAL'
      f.primaryAxisSizingMode = 'AUTO'
      f.counterAxisSizingMode = 'AUTO'
      f.itemSpacing = gap; f.clipsContent = false
      return f
    ...
    async function mkT(text: string, size: number, font: FontDef, color: RGB): Promise<TextNode> {
      const t = figma.createText()
      t.fontName = font; t.fontSize = size; t.fills = [sp(color)]
      t.characters = text || '—'
      t.textAutoResize = 'WIDTH_AND_HEIGHT'
      return t
    }

    function makeTextWrap(t: TextNode): void {
    ...
    async function mkChip(text: string, bg: RGB, color: RGB, fontSize = 11): Promise<FrameNode> {
      const f = mkH('Chip', 0)
      f.fills = [sp(bg)]; f.cornerRadius = 100
    ...
    function mkCard(name: string, gap = 16, borderPaint?: SolidPaint): FrameNode {
      const c = mkV(name, gap)
      c.fills = [sp(cSurf)]; c.cornerRadius = 8
      c.paddingTop = 20; c.paddingBottom = 20; c.paddingLeft = 20; c.paddingRight = 20
      c.strokes = [borderPaint ?? sp(cBorder)]; c.strokeWeight = 1
      return c
    }

    function mkExCont(name: string): FrameNode {
      const f = mkH(name, 0)
      f.fills = [sp(cPrev)]; f.cornerRadius = 8; f.clipsContent = false
      f.paddingTop = 32; f.paddingBottom = 32; f.paddingLeft = 32; f.paddingRight = 32
    ...
    doc.resize(DOC_W, 100)
    doc.primaryAxisSizingMode = 'AUTO'
    doc.counterAxisSizingMode = 'AUTO'
    doc.minWidth = 2100
    doc.maxWidth = 2500
    doc.paddingTop = DOC_PAD; doc.paddingBottom = DOC_PAD * 2
    doc.paddingLeft = DOC_PAD; doc.paddingRight = DOC_PAD
    doc.itemSpacing = 48; doc.clipsContent = false
    const vc = figma.viewport.center
    doc.x = vc.x - DOC_W / 2; doc.y = vc.y - 500

    function docAdd(child: FrameNode | TextNode | RectangleNode): void {
    ...
    // ── Section builder ───────────────────────────────────────────────────────
    async function mkSection(title: string): Promise<FrameNode> {
      const s = mkV(`Section: ${title}`, 28)
      const hdr = mkH(`${title} Header`, 14)
      hdr.counterAxisAlignItems = 'CENTER'

      const bar = figma.createRectangle()
      bar.name = 'Accent'; bar.resize(4, 26)
      bar.fills = [sp(cBlue)]; bar.cornerRadius = 2
      add(hdr, bar)
      ...
      s.appendChild(hdr)
      ;(hdr as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'
      return s
    }
    ...
    // ── HEADER ────────────────────────────────────────────────────────────────
    {
      ...
      hdr.fills = [sp(cSurf)]; hdr.cornerRadius = 12
      hdr.strokes = [sp(cBorder)]; hdr.strokeWeight = 1
      hdr.paddingTop = 40; hdr.paddingBottom = 40; hdr.paddingLeft = 40; hdr.paddingRight = 40

      const titleRow = mkH('Title Row', 16)
      titleRow.counterAxisAlignItems = 'CENTER'
      const cTitle: RGB = { r:0.290, g:0.290, b:0.290 }
      try {
        const iconSrc = await figma.getNodeByIdAsync(HEADER_ICON_NODE_ID)
        if (iconSrc && iconSrc.type === 'INSTANCE') {
          const iconMainComp = await (iconSrc as InstanceNode).getMainComponentAsync()
          if (iconMainComp) { const i = iconMainComp.createInstance(); i.resize(43, 43); add(titleRow, i) }
        } else if (iconSrc && iconSrc.type === 'COMPONENT') {
          const i = (iconSrc as ComponentNode).createInstance(); i.resize(43, 43); add(titleRow, i)
        }
      } catch { /* icon not found — skip silently */ }

      const titleT = await mkT(displayName, 45, fB, cTitle)
      add(titleRow, titleT)
      ;(titleT as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'
      titleT.textAutoResize = 'HEIGHT'

      const sc = STATUS_COLORS[params.status] ?? STATUS_COLORS['Ready']
      const statusChip = mkH('Status', 0)
      statusChip.fills = [sp(sc.bg)]; statusChip.cornerRadius = 6
      statusChip.paddingTop = 15; statusChip.paddingBottom = 15; statusChip.paddingLeft = 15; statusChip.paddingRight = 15
      ...
      if (descText) {
        ...
        addFill(hdr, dt); makeTextWrap(dt)
      }

      if (params.dsLink) {
        const linkChip = mkH('DS Link', 6)
        linkChip.fills = [sp(cSecBg)]; linkChip.cornerRadius = 6
        linkChip.paddingTop = 10; linkChip.paddingBottom = 10; linkChip.paddingLeft = 10; linkChip.paddingRight = 10
      ...
      if (params.dsLink) {
        ...
        addFill(hdr, urlT); makeTextWrap(urlT)
      }
    ...
    // ── USE CASES ─────────────────────────────────────────────────────────────
    const usageText = mdSection(md, ['Usage','Use Cases','Use Case','Guidelines','Best Practices','When to use'])
    if (usageText) {
      ...
      const dos   = mdBullets(usageText, ['Do', 'Dos', "Do's", 'Should', 'Best practice'])
      const donts = mdBullets(usageText, ["Don't","Donts","Don'ts",'Avoid','Do not','Should not'])
      ...
      if (introText || dos.length > 0 || donts.length > 0) {
        ...
        if (dos.length > 0 || donts.length > 0) {
          ...
          if (dos.length > 0) {
            ...
            const doHdr = mkH('Do Hdr', 6); doHdr.counterAxisAlignItems = 'CENTER'
            const dm = await mkT('✔', 20, fB, cGreen); const dl = await mkT('Do', 20, fB, cGreen)
            add(doHdr, dm); add(doHdr, dl); add(doCard, doHdr)
            for (const item of dos.slice(0, 6)) {
              const ir = mkH('Do Item', 8); ir.counterAxisAlignItems = 'MIN'
              const doIcon = await mkT('✔', 20, fR, cGreen)
              const doTxt  = await mkT(item, 20, fR, cText)
              add(ir, doIcon); add(ir, doTxt)
              ir.resize(900, 1); ir.primaryAxisSizingMode = 'FIXED'; ir.counterAxisSizingMode = 'AUTO'
              ;(doTxt as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'
              doTxt.textAutoResize = 'HEIGHT'
              add(doCard, ir)
            }
            row.appendChild(doCard)
            ;(doCard as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'
            ;(doCard as unknown as WithLayoutSizing).layoutSizingVertical = 'FILL'
          }

          if (donts.length > 0) {
            const dontCard = mkCard("Don't", 10, sp(cRed, 0.4))
            const dHdr = mkH("Don't Hdr", 6); dHdr.counterAxisAlignItems = 'CENTER'
            const dm = await mkT('✘', 20, fB, cRed); const dl = await mkT("Don't", 20, fB, cRed)
            add(dHdr, dm); add(dHdr, dl); add(dontCard, dHdr)
            for (const item of donts.slice(0, 6)) {
              const ir = mkH('Dont Item', 8); ir.counterAxisAlignItems = 'MIN'
              const dontIcon = await mkT('✘', 20, fR, cRed)
              const dontTxt  = await mkT(item, 20, fR, cText)
              add(ir, dontIcon); add(ir, dontTxt)
              ir.resize(900, 1); ir.primaryAxisSizingMode = 'FIXED'; ir.counterAxisSizingMode = 'AUTO'
              ;(dontTxt as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'
              dontTxt.textAutoResize = 'HEIGHT'
              add(dontCard, ir)
            }
            row.appendChild(dontCard)
            ;(dontCard as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'
            ;(dontCard as unknown as WithLayoutSizing).layoutSizingVertical = 'FILL'
          }
    ...
    async function renderRichText(container: FrameNode, text: string, fontSize = 13): Promise<void> {
      let paraAcc: string[] = []
      const flushPara = async () => {
        const t = paraAcc.join('\n').trim()
        if (t) { const p = await mkT(t, fontSize, fR, cText); addFill(container, p); makeTextWrap(p) }
        paraAcc = []
      }
      for (const line of text.split('\n')) {
        ...
        if (bm) {
          await flushPara()
          const ir = mkH('Item', 10); ir.counterAxisAlignItems = 'MIN'
    ...
    if (tipsText) {
      ...
      const bullets = tipsText.split('\n').map(l => l.replace(/^[-*+]\s*/,'').trim()).filter(l => l.length > 0)
      if (bullets.length > 1) {
        for (const b of bullets) {
          const ir = mkH('Tip Item', 10); ir.counterAxisAlignItems = 'MIN'
          const arrow = await mkT('→', 20, fM, cBlue)
          const txt = await mkT(b, 20, fR, cText)
          add(ir, arrow); add(ir, txt)
          ;(txt as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'
        ...
        addFill(card, tt); makeTextWrap(tt)
      }
      addFill(s, card)
      docAdd(s)
    }
    ...
    function findVariantDesc(propName: string, value: string): string {
      const parentKeys = [
        propName, propName + 's',
        'Variants', 'Variant', 'Types', 'Type',
        'Appearances', 'Appearance', 'Sizes', 'Size',
        'States', 'State', 'Styles', 'Style',
      ]
      ...
      for (const key of parentKeys) {
        ...
        if (!sc) continue
        const lines = sc.split('\n'); let found = false; const out: string[] = []
        for (const line of lines) {
          if (line.match(subRe)) { found = true; continue }
    ...
    async function addVariantSection(
      sectionTitle: string,
      entries: [string,CProp][],
      cols = 1
    ): Promise<void> {
      if (!entries.length) return
      const s = await mkSection(sectionTitle)
      const cards: FrameNode[] = []
      const exConts: FrameNode[] = []
      ...
      for (const [propName, propData] of entries) {
        for (const value of propData.values) {
          ...
          if (mainComp) {
            ...
            try { inst.setProperties({ [rawKey]: value }) } catch { /* ignore */ }
    ...
    function layoutColumns(maxInstW: number): 1 | 2 {
      return maxInstW > 600 ? 1 : 2
    }

    function maxCompWidth(entries: [string, CProp][]): number {
      let w = 0
      for (const [propName, propData] of entries) {
        for (const value of propData.values) {
    ...
    function buildGridList(name: string, cards: FrameNode[], cols: number = 2): FrameNode {
      ...
      for (let i = 0; i < cards.length; i += n) {
        ...
        if (rowCards.length === 1) {
          ...
          for (const c of rowCards) {
            row.appendChild(c)
            ;(c as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'
          }
          const maxH = Math.max(...rowCards.map(c => c.height))
          if (maxH > 0) {
            for (const c of rowCards) {
              if (c.height < maxH) {
                ;(c as unknown as WithLayoutSizing).layoutSizingVertical = 'FILL'
              }
            }
          }
          addFill(list, row)
        ...
        for (const value of propData.values) {
          ...
          if (showDimensions && inst) {
            const dimT = await mkT(
              `W: ${Math.round(inst.width)}px  •  H: ${Math.round(inst.height)}px`,
              16, fR, cMuted
            )
            addFill(card, dimT); makeTextWrap(dimT)
          }
    ...
    async function buildBoolCard(
      propName: string,
      propData: CProp,
      stackExamples = false
    ): Promise<{ card: FrameNode; exConts: FrameNode[] }> {
      const collectedExConts: FrameNode[] = []
      const card = mkCard(`${propName} Card`, 14)

      const nameT = await mkT(titleCase(propName), 20, fSB, cText)
      addFill(card, nameT); makeTextWrap(nameT)
      ...
      const isVariantBool = propData.type === 'BOOLEAN' && propData.values.length === 2 &&
      ...
      if (stackExamples) {
        for (const [label, propVal] of [['OFF', offVal], ['ON', onVal]] as [string, string|boolean][]) {
          const isOn = label === 'ON'
          ...
          if (mainComp) {
            const inst = mainComp.createInstance()
            try { inst.setProperties({ [propData.rawKey]: propVal }) } catch { /* ignore */ }
            add(instWrapper, inst)
          }
          add(exCont, instWrapper)
          addFill(col, exCont)
        ...
        for (const [label, propVal] of [['OFF', offVal], ['ON', onVal]] as [string, string|boolean][]) {
          ...
          ;(col as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'
        }
        addFill(card, exRow)
      }
    ...
    if (booleanProps.length > 0) {
      ...
      const allBoolExConts: FrameNode[] = []
      for (const [propName, propData] of booleanProps) {
        const { card, exConts: bec } = await buildBoolCard(propName, propData, boolStack)
        addFill(list, card)
    ...
    let themeGridNaturalW = 0

    // ── THEMES ────────────────────────────────────────────────────────────────
    {
      type VarThemeEntry = { col: VariableCollection; modeId: string; modeName: string }
      const varThemes: VarThemeEntry[] = []
      const auditLog: string[] = ['=== Themes Audit ===']
      const THEME_MODE_RE = /\b(modern|legacy|light|dark)\b/i

      function themeHits(col: VariableCollection): {modeId:string; modeName:string}[] {
        return col.modes
          .filter(m => THEME_MODE_RE.test(m.name))
          .map(m => ({ modeId: m.modeId, modeName: m.name }))
      }
      function addToVarThemes(col: VariableCollection, source: string): void {
        const hits = themeHits(col)
        if (hits.length >= 2) {
          for (const h of hits) varThemes.push({ col, modeId: h.modeId, modeName: h.modeName })
          auditLog.push(`  → "${col.name}" (${source}): ${hits.map(h => h.modeName).join(', ')}`)
        }
      }

      auditLog.push('Strategy A: resolvedVariableModes on component')
      try {
        const srcNode = compSet ?? mainComp ?? (sel.type !== 'COMPONENT_SET' ? sel : null)
        const modesMap: {[colId: string]: string} = (srcNode as SceneNode & { resolvedVariableModes?: {[k:string]:string} })?.resolvedVariableModes ?? {}
        const collectionIds = Object.keys(modesMap)
        auditLog.push(`  resolvedVariableModes: ${collectionIds.length} collection(s)`)
        for (const colId of collectionIds) {
          const col = await figma.variables.getVariableCollectionByIdAsync(colId)
          if (col) {
            auditLog.push(`  collection "${col.name}" (${col.remote ? 'remote' : 'local'}): modes=[${col.modes.map(m=>m.name).join(', ')}]`)
        ...
        auditLog.push(`  Strategy A error: ${e instanceof Error ? e.message : String(e)}`)
      }
      ...
      if (varThemes.length < 2) {
        ...
        try {
          ...
          for (const col of localCols) {
            auditLog.push(`  "${col.name}" (${col.remote ? 'remote' : 'local'}): [${col.modes.map(m=>m.name).join(', ')}]`)
            addToVarThemes(col, 'local/imported')
          }
        } catch (e) {
          auditLog.push(`  Strategy B error: ${e instanceof Error ? e.message : String(e)}`)
        }
        auditLog.push(`  After B: ${varThemes.length} theme mode(s)`)
      }

      if (varThemes.length < 2) {
        auditLog.push('Strategy C: teamLibrary.getAvailableLibraryVariableCollectionsAsync')
        try {
          ...
          for (const libCol of libCols) {
            ...
            try {
              ...
              if (libVars.length > 0) {
                ...
                if (col) {
                  auditLog.push(`    imported → "${col.name}": [${col.modes.map(m=>m.name).join(', ')}]`)
                  addToVarThemes(col, 'teamLibrary import')
                }
              }
            } catch (importErr) {
              auditLog.push(`    import error: ${importErr instanceof Error ? importErr.message : String(importErr)}`)
            }
          }
        } catch (e) {
          auditLog.push(`  Strategy C error: ${e instanceof Error ? e.message : String(e)}`)
        }
        auditLog.push(`  After C: ${varThemes.length} theme mode(s)`)
      }

      interface ColorToken { varId: string; name: string; color: RGBA }
      ...
      async function getVar(id: string): Promise<Variable | null> {
        ...
        try { const v = await figma.variables.getVariableByIdAsync(id); varCache.set(id, v); return v }
        catch { varCache.set(id, null); return null }
      }

      function isRGBA(v: VariableValue): v is RGBA {
        return typeof v === 'object' && v !== null && 'r' in v && 'g' in v && 'b' in v
      }
      function isVarAlias(v: VariableValue): v is VariableAlias {
        return typeof v === 'object' && v !== null && (v as VariableAlias).type === 'VARIABLE_ALIAS'
      }

      async function resolveColorVar(varId: string, consumer: SceneNode): Promise<RGBA | null> {
        ...
        try {
          ...
          if (resolved.resolvedType === 'COLOR' && isRGBA(resolved.value)) return resolved.value as RGBA
          if (isVarAlias(resolved.value)) return resolveColorVar(resolved.value.id, consumer)
        } catch { /* unresolvable */ }
        return null
      }

      async function extractColorTokens(inst: InstanceNode): Promise<ColorToken[]> {
        const seen = new Set<string>()
        const tokens: ColorToken[] = []

        async function addToken(vid: string): Promise<void> {
        ...
        async function visit(node: SceneNode, depth: number): Promise<void> {
          if (depth > 12) return

          for (const paintArray of ['fills', 'strokes'] as const) {
            if (!(paintArray in node)) continue
            const paints = (node as GeometryMixin)[paintArray] as readonly Paint[]
          ...
          try {
            const nbv = (node as SceneNode & { boundVariables?: Record<string, VariableAlias | VariableAlias[]> }).boundVariables
            if (nbv) {
          ...
          if ('effects' in node) {
            ...
            if (Array.isArray(effects)) {
              for (const eff of effects) {
                const ebv = (eff as { boundVariables?: { color?: VariableAlias } }).boundVariables
          ...
          if ('children' in node) {
            for (const child of (node as FrameNode).children) {
              await visit(child as SceneNode, depth + 1)
            }
          }
        }
        ...
        function tokenPriority(name: string): number {
      ...
      function extractHardcodedColors(inst: SceneNode): ColorToken[] {
        const seen = new Set<string>()
        const tokens: ColorToken[] = []

        function visitHardcoded(node: SceneNode, depth: number): void {
          if (depth > 12) return
          ...
          for (const paintArray of ['fills', 'strokes'] as const) {
            ...
            for (const paint of paints) {
              if (paint.type !== 'SOLID') continue
              if ((paint as SolidPaint).visible === false) continue
              const c = (paint as SolidPaint).color
              const opacity = (paint as SolidPaint).opacity ?? 1
              if (opacity < 0.01) continue
              const hex = colorToHex({ r: c.r, g: c.g, b: c.b, a: opacity })
              if (seen.has(hex)) continue
              seen.add(hex)
              tokens.push({ varId: hex, name: hex, color: { r: c.r, g: c.g, b: c.b, a: opacity } })
            }
          }

          if ('children' in node) {
            for (const child of (node as FrameNode).children) {
              visitHardcoded(child as SceneNode, depth + 1)
            }
          }
        }
      ...
      async function renderTokenList(
        ...
        for (const tok of tokens) {
          ...
          swatch.name = 'Swatch'; swatch.resize(12, 12)
          swatch.fills = [{ type: 'SOLID', color: { r: tok.color.r, g: tok.color.g, b: tok.color.b }, opacity: tok.color.a ?? 1 }]
          swatch.strokes = [{ type: 'SOLID', color: { r: 0, g: 0, b: 0 }, opacity: 0.12 }]
          swatch.strokeWeight = 0.5
          add(row, swatch)

          const nameT = figma.createText()
          await figma.loadFontAsync(fM)
          nameT.fontName = fM; nameT.fontSize = 11; nameT.fills = [sp(textCol)]
          nameT.characters = tok.name; nameT.textAutoResize = 'WIDTH_AND_HEIGHT'
          add(row, nameT)

          const hexT = figma.createText()
          await figma.loadFontAsync(fR)
          hexT.fontName = fR; hexT.fontSize = 11; hexT.fills = [sp(mutedCol)]
          hexT.characters = colorToHex(tok.color); hexT.textAutoResize = 'WIDTH_AND_HEIGHT'
          add(row, hexT)
      ...
      const dedupedThemes = varThemes.filter((t, i, arr) =>
      ...
      if (dedupedThemes.length >= 2) {
        ...
        const cDarkColBg:  RGB = { r:0.08,  g:0.08,  b:0.10  }
        const cDarkBorder: RGB = { r:0.18,  g:0.18,  b:0.22  }
        const cDarkText:   RGB = { r:0.94,  g:0.94,  b:0.97  }
        const cDarkMuted:  RGB = { r:0.60,  g:0.60,  b:0.68  }
        const cDarkExBg:   RGB = { r:0.12,  g:0.12,  b:0.16  }
        const cLightColBg: RGB = { r:1,     g:1,     b:1     }
        const cLightBorder: RGB = cBorder

        function isDark(modeName: string): boolean { return /dark/i.test(modeName) }
        ...
        for (const vt of dedupedThemes) {
          ...
          col.fills = [sp(colBg)]; col.cornerRadius = 12
          col.strokes = [sp(colBorder)]; col.strokeWeight = 1
          col.paddingTop = 20; col.paddingBottom = 20; col.paddingLeft = 20; col.paddingRight = 20
          col.clipsContent = false

          const colTitle = await mkT(formatThemeLabel(vt.modeName), 20, fSB, titleCol)
          add(col, colTitle)

          const sep = figma.createRectangle()
          sep.name = 'Sep'; sep.resize(40, 1)
          ...
          if (appearanceProps.length > 0) {
            for (const [propName, propData] of appearanceProps) {
              for (const value of propData.values) {
                ...
                exCont.fills = [sp(exBg)]; exCont.cornerRadius = 8
                exCont.paddingTop = 20; exCont.paddingBottom = 20
                exCont.paddingLeft = 20; exCont.paddingRight = 20
                exCont.primaryAxisAlignItems = 'CENTER'
                exCont.counterAxisAlignItems = 'CENTER'
                exCont.clipsContent = false

                let themeInst: InstanceNode | null = null
                ...
                if (comp) {
                  try {
                    ...
                    const rawKey = componentProps.get(propName)?.rawKey ?? propName
                    try { inst.setProperties({ [rawKey]: value }) } catch { /* ignore */ }
                    const iw = mkH('IW', 0); add(iw, inst); add(exCont, iw)
                  } catch (e) {
                    const et = await mkT(`Error: ${e instanceof Error ? e.message : String(e)}`, 11, fR, varTxtCol)
                    add(exCont, et)
                  }
                }
                ...
                if (themeInst) {
                  try {
                    ...
                    if (tokens.length === 0) tokens = extractHardcodedColors(themeInst)
                    await renderTokenList(varBlock, tokens, varTxtCol, dark ? cDarkMuted : cMuted)
                  } catch { /* token extraction optional — skip on error */ }
                }
            ...
            exCont.paddingTop = 20; exCont.paddingBottom = 20
            exCont.paddingLeft = 20; exCont.paddingRight = 20
            exCont.primaryAxisAlignItems = 'CENTER'; exCont.counterAxisAlignItems = 'CENTER'
            exCont.clipsContent = false
            let themeInst: InstanceNode | null = null
            if (mainComp) {
              try {
    ...
    const a11y = mdSection(md, [
      'Accessibility','A11y','Screen Reader','Screen Reader Support',
      'Keyboard','Keyboard Navigation','Focus','Focus Management','ARIA',
    ])
    if (a11y) {
    ...
    if (themeGridNaturalW > 0) {
      const requiredW = Math.min(1800, Math.max(DOC_W, themeGridNaturalW + DOC_PAD * 2))
      if (requiredW > doc.width) {
        doc.resize(requiredW, 100)
        doc.primaryAxisSizingMode = 'AUTO'
    ...
    // ── FOOTER ────────────────────────────────────────────────────────────────
    {
      ...
      div.fills = [sp(cBorder)]; div.name = 'Divider'
      doc.appendChild(div)
      ;(div as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'

      const ft = await mkT(
        `Documentation generator  •  ${displayName}  •  ${new Date().toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'})}`,
        15, fR, cMuted
      )
      addFill(doc, ft); makeTextWrap(ft)
    }

    // ── RAW MARKDOWN TEXT NODE ─────────────────────────────────────────────────
    if (md && md.trim().length > 0) {
      function cleanAndReorderMd(raw: string): string {
        ...
        for (const s of sections) {
          if (!s.title) { if (s.content.length > 0) merged.push(s); continue }
        ...
        const order = ['usage', 'tips', 'tip', 'variants', 'appearance', 'accessibility', 'seealso', 'see also', 'themes']
      ...
      mdFrame.paddingTop = 40; mdFrame.paddingBottom = 40
      mdFrame.paddingLeft = 40; mdFrame.paddingRight = 40
      ...
      ;(mdText as unknown as WithLayoutSizing).layoutSizingHorizontal = 'FILL'
...
async function runAction_generate(target: SceneNode, notify: boolean): Promise<void> {
  isExecuting = true
  try {
...
function pushActionStates(): void {
  ...
  figma.ui.postMessage({
    type: 'action-state',
    actions: { generate: { enabled: t != null, label: 'Generate documentation' } },
  })
}

function sendComponentInfo(): void {
  const target = singleSelectedTarget()
  if (!target) {
    figma.ui.postMessage({ type:'component-info', name:'', variantCount:0, propNames:'' }); return
  }
  let name = ''; let variantCount = 0; let propNames = ''
  if (target.type === 'COMPONENT_SET') {
    const cs = target as ComponentSetNode
    name = cs.name; variantCount = cs.children.length
    if ('componentPropertyDefinitions' in cs)
      propNames = Object.keys(cs.componentPropertyDefinitions).map(k => k.replace(/#\d+$/,'')).join(', ')
  } else if (target.type === 'COMPONENT') {
    const c = target as ComponentNode
    name = c.parent?.type === 'COMPONENT_SET' ? (c.parent as ComponentSetNode).name : c.name
    variantCount = c.parent?.type === 'COMPONENT_SET' ? (c.parent as ComponentSetNode).children.length : 1
    const ss = c.parent?.type === 'COMPONENT_SET' ? (c.parent as ComponentSetNode) : c
    if ('componentPropertyDefinitions' in ss)
      propNames = Object.keys(ss.componentPropertyDefinitions).map(k => k.replace(/#\d+$/,'')).join(', ')
  } else if (target.type === 'INSTANCE') {
    name = target.name; variantCount = 1
  } else {
    name = target.name
  }
  figma.ui.postMessage({ type:'component-info', name, variantCount, propNames })
}
...
let html = __html__
html = html.replace(/(id="dsLink"[^>]*\bvalue=")[^"]*(")/g, '$1' + htmlEsc(initialParams.dsLink) + '$2')
...
figma.ui.onmessage = (msg: RunMsg) => {
  if (msg.type === 'resize') {
    figma.ui.resize(300, Math.max(200, Math.min(900, Math.round(msg.height)))); return
  }
  ...
  if (msg.type === 'get-gh-token') {
    void figma.clientStorage.getAsync('gh-token').then((token: string | undefined) => {
  ...
  if (msg.type === 'get-github-info') {
    const target = singleSelectedTarget()
    if (!target) {
      figma.ui.postMessage({ type: 'github-info', url: '', sha: '', generatedAt: '' })
      return
    }
    const page = figma.currentPage
    let docFrame: SceneNode | null = null
    for (const child of page.children) {
      if (child.type === 'FRAME' && child.getPluginData('github-sha')) {
        docFrame = child; break
      }
    }
    if (docFrame && docFrame.type === 'FRAME') {
      figma.ui.postMessage({
        type: 'github-info',
        url: (docFrame as FrameNode).getPluginData('github-url'),
        sha: (docFrame as FrameNode).getPluginData('github-sha'),
        generatedAt: (docFrame as FrameNode).getPluginData('github-generated-at'),
      })
    } else {
      figma.ui.postMessage({ type: 'github-info', url: '', sha: '', generatedAt: '' })
    }
